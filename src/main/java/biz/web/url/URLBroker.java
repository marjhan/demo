...
import biz.web.velocity.DirectOutput;
...
public class URLBroker {
	...
	public class QueryData implements DirectOutput {
		...
		public QueryData addQueryData(String id, String value) {
			...
			try {
				...
				if (logger.isErrorEnabled()) {
					logger.error("UnsupportedEncoding:" + URLBroker.this.encoding, e);
				}
			}
			query.append('&');
			return this;
		...
		public String toString() {
			...
			if (last == '?' || last == '&') {
				return this.query.deleteCharAt(this.query.length() - 1).toString();
			}
